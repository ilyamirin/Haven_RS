...
Ниже — упрощённая псевдографическая схема графа (LangGraph). Узлы исполняются последовательно от подготовки до координатора.
...
```
Агент,Роль,Личность,Поддержка tooling?
Color Expert,"Подбирает палитру","Вежливый, аккуратный",no
Style Expert,"Рекомендует стили","Дерзкий, уверенный",no
Wardrobe Agent,"Координатор","Собирает мнения и выбирает итог",no
```
Ключевые заголовки (кейсы и пробелы не критичны): Агент/Agent/Name, Роль/Role, Личность/Persona, Поддержка tooling?/Tooling/Supports tooling.
...
```
В обоих случаях последний агент — координатор (если в списке не встречается «Wardrobe Agent»).
